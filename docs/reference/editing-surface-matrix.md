...
| Timing nodes | `setPresentationTimingNodes` | Persisted | Parser/editor/serializer preserve core `p:cTn` metadata plus restart/fill policy, start-condition trigger event/delay fields, and target shape IDs. |
...
- PPTX richer animation graph editing beyond core timing/start-condition/target/restart metadata
