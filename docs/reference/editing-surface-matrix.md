...
| Timing nodes | `setPresentationTimingNodes` | Persisted | Parser/editor/serializer preserve core `p:cTn` metadata plus repeat duration/auto-reverse, restart/fill policy, accel/decel timing curves, start/end-condition trigger event/delay fields, trigger-shape bindings, and target shape IDs. |
