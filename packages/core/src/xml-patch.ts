...
export function replaceContainerTextByAttribute(source: string, options: { tagName: string; newText: string; keyAttr?: string; keyValue?: string; occurrence?: number }): string {
  const tagPattern = new RegExp(`(<${escapeRegExp(options.tagName)}\\b[^>]*>)([\\s\\S]*?)(<\\/${escapeRegExp(options.tagName)}>)`, 'g');
  let matchedCount = 0;

  return source.replace(tagPattern, (full, openTag, _content, closeTag) => {
    if (options.keyAttr && options.keyValue !== undefined) {
      const keyMatch = new RegExp(`${escapeRegExp(options.keyAttr)}=(["'])(.*?)\\1`).exec(openTag);
      if (!keyMatch || keyMatch[2] !== options.keyValue) {
        return full;
      }
    }
    ...
    return `${openTag}${escapeXml(options.newText)}${closeTag}`;
  });
}
