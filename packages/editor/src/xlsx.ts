...
export function setWorksheetChartBarDirection(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, barDirection: string | undefined): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
...
export function setWorksheetChartRoundedCorners(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, roundedCorners: boolean | undefined): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
...
export function setWorksheetChartCategoryAxisPosition(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, position: string | undefined): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
      chart.categoryAxisPosition = position;
    }
  });
}
...
export function setWorksheetChartCategoryAxisMajorGridlines(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, enabled: boolean | undefined): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
      chart.categoryAxisMajorGridlines = enabled;
    }
  });
}

export function setWorksheetChartCategoryAxisMinorGridlines(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, enabled: boolean | undefined): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
      chart.categoryAxisMinorGridlines = enabled;
    }
  });
}
...
export function setWorksheetChartCategoryAxisTickLabelSkip(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, tickLabelSkip: number | undefined): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
      chart.categoryAxisTickLabelSkip = tickLabelSkip;
    }
  });
}
...
export function setWorksheetChartValueAxisMajorGridlines(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, enabled: boolean | undefined): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
      chart.valueAxisMajorGridlines = enabled;
    }
  });
}

export function setWorksheetChartValueAxisMinorGridlines(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, enabled: boolean | undefined): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
      chart.valueAxisMinorGridlines = enabled;
    }
  });
}
...
export function setWorksheetChartDataLabelVisibility(editor: OfficeEditor<XlsxWorkbook>, sheetName: string, chartIndex: number, visibility: { showValue?: boolean; showCategoryName?: boolean; showSeriesName?: boolean; showLegendKey?: boolean; showLeaderLines?: boolean; showPercent?: boolean; showBubbleSize?: boolean; separator?: string }): XlsxWorkbook {
  return editor.transaction((draft) => {
    const sheet = draft.sheets.find((entry) => entry.name === sheetName);
    const chart = sheet?.charts[chartIndex];
    if (chart) {
