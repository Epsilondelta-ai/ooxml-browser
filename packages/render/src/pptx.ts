...
export function renderPresentation(presentation: PresentationDocument, options: RenderOptions): string {
  ...
  const shapeMarkup = slide.shapes.map((shape) => `<div class="ooxml-pptx-shape"${shape.placeholderType ? ` data-placeholder-type="${escapeHtml(shape.placeholderType)}"` : ''}${shape.media?.targetUri ? ` data-media-uri="${escapeHtml(shape.media.targetUri)}"` : ''}${shape.media?.type ? ` data-media-type="${escapeHtml(shape.media.type)}"` : ''}${shape.media?.progId ? ` data-prog-id="${escapeHtml(shape.media.progId)}"` : ''}${shape.transform?.x !== undefined ? ` data-x="${shape.transform.x}"` : ''}${shape.transform?.y !== undefined ? ` data-y="${shape.transform.y}"` : ''}${shape.transform?.cx !== undefined ? ` data-cx="${shape.transform.cx}"` : ''}${shape.transform?.cy !== undefined ? ` data-cy="${shape.transform.cy}"` : ''}><h3>${escapeHtml(shape.name ?? 'Shape')}</h3><p>${escapeHtml(shape.text || (shape.media?.type === 'embeddedObject' ? '[embedded-object]' : shape.media ? '[image]' : ''))}</p></div>`).join('');
