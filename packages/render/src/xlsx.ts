...
export function renderWorkbook(workbook: XlsxWorkbook, options: RenderOptions): string {
  ...
  const chartsMarkup = sheet.charts.length ? `<p class="ooxml-xlsx-charts">Charts: ${sheet.charts.map((chart) => `${escapeHtml(chart.name ?? chart.relationshipId)}${chart.title ? ` [${escapeHtml(chart.title)}]` : ''} (${escapeHtml(chart.targetUri)})`).join(', ')}</p>` : '';
