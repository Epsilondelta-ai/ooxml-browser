...
2. Confirm corpus/package correctness before tuning slide cosmetics.
...
### Decision drivers
1. Corpus-wide work is only trustworthy if every sample deck parses with the expected slide counts.
...
### Decision
Choose **Option A**: confirm corpus correctness, add richer PPTX scene semantics in the renderer, and validate them with a screenshot/reference harness.
...
### Expanded test plan
- **Unit:** package-opening regressions, background/theme extraction, shape/image projection helpers
...
### Stage 1 — corpus/package correctness
Deliverables:
- verify and lock correct PPTX opening / slide counts for all six samples
- tests covering corpus-opening regressions and slide-count expectations
...
- incorrect corpus counts or silent deck degradation blocks trust in corpus-wide work
...
## Verification path
1. parse counts for sample1-6 match the paired PNG corpus
