...
describe('pptx timing and transitions', () => {
  it('parses slide transition and timing node metadata', async () => {
    ...
    expect(slide?.timing?.nodes).toEqual([
      { nodeType: 'par', presetClass: 'entr', presetId: '1', id: '10', duration: '500', repeatCount: '1', triggerEvent: 'onBegin', triggerDelay: '0' },
      { nodeType: 'seq', presetClass: 'exit', presetId: '2', id: '20', duration: '750', repeatCount: 'indefinite', triggerEvent: 'onClick', triggerDelay: '250' }
    ]);
  });
  ...
  it('renders timing and transition metadata', async () => {
    ...
    expect(html).toContain('par:entr#10@500×1!onBegin+0, seq:exit#20@750×indefinite!onClick+250');
  });
});
