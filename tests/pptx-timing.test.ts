...
describe('pptx timing and transitions', () => {
  it('parses slide transition and timing node metadata', async () => {
    ...
    expect(slide?.timing?.nodes).toMatchObject([
      ...
      { nodeType: 'animMotion', presetClass: 'path', presetId: '7', id: '50', duration: '550', repeatDuration: '900', repeatCount: '1', restart: 'always', fill: 'hold', autoReverse: false, acceleration: '150', deceleration: '250', motionOrigin: 'layout', motionPath: 'M 0 0 L 0.25 0.25', motionPathEditMode: 'relative', triggerEvent: 'onClick', triggerDelay: '5', triggerShapeId: '3', endTriggerEvent: 'afterEffect', endTriggerDelay: '25', endTriggerShapeId: '4', targetShapeId: '2' },
      ...
      { nodeType: 'cmd', presetClass: 'verb', presetId: '6', id: '45', duration: '250', repeatDuration: '500', repeatCount: '1', restart: 'always', fill: 'hold', autoReverse: false, acceleration: '200', deceleration: '300', commandName: 'togglePause', commandType: 'call', triggerEvent: 'withPrevious', triggerDelay: '10', triggerShapeId: '3', endTriggerEvent: 'onEnd', endTriggerDelay: '15', endTriggerShapeId: '4', targetShapeId: '2' }
    ]);
  });
  ...
  it('renders timing and transition metadata', async () => {
    ...
    expect(html).toContain('data-timing-count="6"');
    expect(html).toContain('par:entr#10@500 repeatDur:1500×1 restart:always fill:hold autoRev:true accel:10000 decel:20000!onBegin+0^3 ~onEnd=50^4->2, seq:exit concurrent:true next:seek prev:none#20@750 repeatDur:2000×indefinite restart:whenNotActive fill:freeze autoRev:false accel:0 decel:5000!onClick+250^3 ~afterEffect=400^4->2, animClr:emph#30@600 repeatDur:900×1 restart:always fill:hold autoRev:false accel:2000 decel:3000 clr:rgb dir:cw!withPrevious+50^3 ~onEnd=75^4->2, animMotion:path#50@550 repeatDur:900×1 restart:always fill:hold autoRev:false accel:150 decel:250 origin:layout path:M 0 0 L 0.25 0.25 pathEdit:relative!onClick+5^3 ~afterEffect=25^4->2, set:set#35@300 repeatDur:600×1 restart:never fill:hold autoRev:false accel:500 decel:700!onClick+20^3 ~afterEffect=40^4->2, cmd:verb#45@250 repeatDur:500×1 restart:always fill:hold autoRev:false accel:200 decel:300 cmd:togglePause cmdType:call!withPrevious+10^3 ~onEnd=15^4->2');
  });
});
