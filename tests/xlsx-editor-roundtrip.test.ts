...
describe('xlsx editor round-trips', () => {
  ...
  it('persists worksheet chart bar-direction edits through save flows', async () => {
    const editor = createOfficeEditor(parseXlsx(await openPackage(createChartedXlsxFixture())));
    setWorksheetChartBarDirection(editor, 'Sheet1', 0, 'bar');

    const serialized = serializeOfficeDocument(editor.document);
    const reopened = parseXlsx(await openPackage(serialized));
    const reopenedGraph = await openPackage(serialized);

    expect(reopened.sheets[0]?.charts[0]?.barDirection).toBe('bar');
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('barDir val="bar"');
  });

  it('persists worksheet chart style edits through save flows', async () => {
    const editor = createOfficeEditor(parseXlsx(await openPackage(createChartedXlsxFixture())));
    setWorksheetChartStyle(editor, 'Sheet1', 0, 204);

    const serialized = serializeOfficeDocument(editor.document);
    const reopened = parseXlsx(await openPackage(serialized));
    const reopenedGraph = await openPackage(serialized);

    expect(reopened.sheets[0]?.charts[0]?.chartStyle).toBe(204);
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('<c:style val="204"');
  });

  it('persists worksheet chart auto-title-deleted edits through save flows', async () => {
    const editor = createOfficeEditor(parseXlsx(await openPackage(createChartedXlsxFixture())));
    setWorksheetChartAutoTitleDeleted(editor, 'Sheet1', 0, true);

    const serialized = serializeOfficeDocument(editor.document);
    const reopened = parseXlsx(await openPackage(serialized));
    const reopenedGraph = await openPackage(serialized);

    expect(reopened.sheets[0]?.charts[0]?.autoTitleDeleted).toBe(true);
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('autoTitleDeleted val="1"');
  });

  it('persists worksheet chart title-overlay edits through save flows', async () => {
    const editor = createOfficeEditor(parseXlsx(await openPackage(createChartedXlsxFixture())));
    setWorksheetChartTitleOverlay(editor, 'Sheet1', 0, true);

    const serialized = serializeOfficeDocument(editor.document);
    const reopened = parseXlsx(await openPackage(serialized));
    const reopenedGraph = await openPackage(serialized);

    expect(reopened.sheets[0]?.charts[0]?.titleOverlay).toBe(true);
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('<c:overlay val="1"');
  });
  ...
  it('persists worksheet line-family decoration edits through save flows', async () => {
    const editor = createOfficeEditor(parseXlsx(await openPackage(createChartedXlsxFixture())));
    setWorksheetChartType(editor, 'Sheet1', 0, 'lineChart');
    setWorksheetChartDropLines(editor, 'Sheet1', 0, false);
    setWorksheetChartHiLowLines(editor, 'Sheet1', 0, true);
    setWorksheetChartSeriesLines(editor, 'Sheet1', 0, false);
    setWorksheetChartUpDownBars(editor, 'Sheet1', 0, true);
    ...
    expect(reopened.sheets[0]?.charts[0]?.chartType).toBe('lineChart');
    expect(reopened.sheets[0]?.charts[0]?.dropLines).toBe(false);
    expect(reopened.sheets[0]?.charts[0]?.hiLowLines).toBe(true);
    expect(reopened.sheets[0]?.charts[0]?.seriesLines).toBe(false);
    expect(reopened.sheets[0]?.charts[0]?.upDownBars).toBe(true);
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('<c:lineChart>');
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('dropLines val="0"');
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('hiLowLines val="1"');
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('serLines val="0"');
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('upDownBars val="1"');
  });
  ...
  it('persists worksheet chart axis-crossing edits through save flows', async () => {
    ...
    setWorksheetChartCategoryAxisMajorGridlines(editor, 'Sheet1', 0, false);
    setWorksheetChartCategoryAxisMinorGridlines(editor, 'Sheet1', 0, true);
    setWorksheetChartCategoryAxisMajorTickMark(editor, 'Sheet1', 0, 'out');
    setWorksheetChartCategoryAxisTickMarkSkip(editor, 'Sheet1', 0, 4);
    ...
    setWorksheetChartCategoryAxisTickLabelSkip(editor, 'Sheet1', 0, 3);
    setWorksheetChartCategoryAxisLabelAlignment(editor, 'Sheet1', 0, 'r');
    setWorksheetChartCategoryAxisNoMultiLevelLabels(editor, 'Sheet1', 0, false);
    ...
    expect(reopened.sheets[0]?.charts[0]?.categoryAxisMajorGridlines).toBe(false);
    expect(reopened.sheets[0]?.charts[0]?.categoryAxisMinorGridlines).toBe(true);
    expect(reopened.sheets[0]?.charts[0]?.categoryAxisMajorTickMark).toBe('out');
    expect(reopened.sheets[0]?.charts[0]?.categoryAxisTickMarkSkip).toBe(4);
    ...
    expect(reopened.sheets[0]?.charts[0]?.categoryAxisTickLabelSkip).toBe(3);
    expect(reopened.sheets[0]?.charts[0]?.categoryAxisLabelAlignment).toBe('r');
    expect(reopened.sheets[0]?.charts[0]?.categoryAxisNoMultiLevelLabels).toBe(false);
    ...
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('tickMarkSkip val="4"');
    ...
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('tickLblSkip val="3"');
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('lblAlgn val="r"');
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('noMultiLvlLbl val="0"');
  ...
  it('persists worksheet chart legend-overlay edits through save flows', async () => {
    const editor = createOfficeEditor(parseXlsx(await openPackage(createChartedXlsxFixture())));
    setWorksheetChartLegendOverlay(editor, 'Sheet1', 0, true);

    const serialized = serializeOfficeDocument(editor.document);
    const reopened = parseXlsx(await openPackage(serialized));
    const reopenedGraph = await openPackage(serialized);

    expect(reopened.sheets[0]?.charts[0]?.legendOverlay).toBe(true);
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('overlay val="1"');
  });

  it('persists worksheet chart rounded-corner edits through save flows', async () => {
    const editor = createOfficeEditor(parseXlsx(await openPackage(createChartedXlsxFixture())));
    setWorksheetChartRoundedCorners(editor, 'Sheet1', 0, false);

    const serialized = serializeOfficeDocument(editor.document);
    const reopened = parseXlsx(await openPackage(serialized));
    const reopenedGraph = await openPackage(serialized);

    expect(reopened.sheets[0]?.charts[0]?.roundedCorners).toBe(false);
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('roundedCorners val="0"');
  });
  ...
  it('persists worksheet chart plot-visible and blanks-as edits through save flows', async () => {
    ...
    setWorksheetChartShowDataLabelsOverMax(editor, 'Sheet1', 0, false);
    ...
    expect(reopened.sheets[0]?.charts[0]?.showDataLabelsOverMax).toBe(false);
    ...
    expect(reopenedGraph.parts['/xl/charts/chart1.xml']?.text).toContain('showDLblsOverMax val="0"');
  });
